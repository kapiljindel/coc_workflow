...
# --- NEW: Accept the chosen_side_name from tactics.py ---
def deploy_swarm(troop_name, category, bunches, all_side_points, panic_points, chosen_side_name):
    ...
    # --- NEW: CALCULATE BASE CENTER FOR SPELL PROJECTION ---
    # We mathematically average all red boundaries to find the absolute center of the enemy base
    ...
    for i in range(len(bunches)):
        ...
        # --- NEW: SPELL DEEP-STRIKE MATH ---
        if category == "spell":
            # Push the drop point 55% to 75% of the way toward the center of the base!
            ...
            # Add a wider scatter for spells so they blanket the buildings
            drop_x += random.randint(-40, 40)
            drop_y += random.randint(-40, 40)
        else:
            # Regular troops stay safely on the outer grass line
            ...
            if calib_img is not None:
                for coord in final_drop_coords:
                    if coord is not None:
                        # Draw Spells as PURPLE dots, and Troops/Heroes as YELLOW dots
    ...
    # --- CATEGORY: SPELLS (Instant Taps) ---
    if category == "spell":
        for i in range(len(bunches)):
            if final_drop_coords[i]:
                dx, dy = final_drop_coords[i]
                device.shell(f"input tap {dx} {dy}")
                time.sleep(random.uniform(0.05, 0.1)) # Lightning fast!
        print(f" [Deploy] SUCCESS: {troop_name} spells deep-striked into buildings.")
        return True
    ...
    while i < len(bunches):
        ...
        if category == "hero":
            device.shell(f"input tap {drop_x} {drop_y}")
            time.sleep(random.uniform(0.05, 0.1))
            i += 1
            continue
            
        # Troops get much faster swipe holds now
        hold_time = random.randint(70, 110) + (troops_to_drop * random.randint(20, 30))
        ...
        if find_and_click("red_area_error.png", click=False, threshold=0.75):
            panic_pt = random.choice(panic_points)
            device.shell(f"input tap {panic_pt[0]} {panic_pt[1]}")
            time.sleep(0.1)
    ...
    # --- HYPER-FAST VERIFICATION (TROOPS ONLY) ---
    if category == "troop":
        time.sleep(0.2) # Dramatically reduced wait time
        ...
        if not os.path.exists(end_img_path):
            ...
            for retry in range(3):
                ...
                panic_pt = random.choice(panic_points)
                # CRITICAL SPEED FIX: Panic swipe reduced from 3000ms to 200ms!
