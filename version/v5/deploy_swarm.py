...
def deploy_swarm(troop_name, category, bunches, all_side_points, panic_points, chosen_side_name):
    ...
    while i < len(bunches):
        ...
        # --- NEW HERO LOGIC: 2-3 Random Anti-Pattern Taps ---
        if category == "hero":
            tap_count = random.randint(2, 3)
            for _ in range(tap_count):
                # Pick a completely random safe dot on this side for EVERY tap
                ...
                time.sleep(random.uniform(0.02, 0.05)) # Lightning fast taps
            
            i += 1
            continue
            
        # --- NEW TROOP LOGIC: Uncapped Machine-Gun Speed ---
        ...
        # We completely removed the red-zone screenshot check here! 
        # The bot just sleeps for a microscopic fraction of a second and fires the next troop.
    ...
    if category == "troop":
        ...
        if not os.path.exists(end_img_path):
            ...
            for retry in range(2): # Only try twice to save time
