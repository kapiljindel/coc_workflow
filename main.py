...
try:
    while True:
        ...
        # --- NEW LOGIC: Check if the start sequence actually worked ---
...
# def rest_countdown(seconds):
#     print() # Add a blank line for visual space
#     while seconds > 0:
#         mins, secs = divmod(seconds, 60)
#         timer = f"{mins:02d}:{secs:02d}"
        
#         # The '\r' at the end makes it overwrite the current line instead of spamming new lines!
#         print(f" [Rest] Cooling down to avoid detection... Resuming in {timer}   ", end="\r")
#         time.sleep(1)
#         seconds -= 1
...
#         # --- FIXED: SEPARATE FLAGS FOR ATTACKING VS RESTING ---
#         attack_completed = False
#         rest_triggered = False
#         next_counter = 0 
...
#                 # Mark as rested so we break the loop cleanly
#                 rest_triggered = True
#                 break 
...
#         # --- FIXED LOGIC ---
#         if attack_completed:
#             # ONLY try to return home if we actually finished an attack. 
#             # If we rested, we are already home!
#             attack_end.return_home()
            
#         # We can check storages safely whether we attacked or just woke up from a rest
#         cg_storage_checker.check_home_loot()
